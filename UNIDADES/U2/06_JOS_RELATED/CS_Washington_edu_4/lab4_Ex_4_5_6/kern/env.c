...
env_setup_vm(struct Env *e)
{
	...
	e->env_pgdir=(pde_t*)page2kva(p);  /*set e->env_pgdir and initialize the page directory*/
	for(i=0;i<PDX(UTOP);i++)           /*from 0 to UTOP mark as zero*/
	  e->env_pgdir[i]=0; 
	/*Copying everything from UTOP upwards. We are just filling up
	  the page directory and not creating any page tables. Those page tables already exist; 
	  we created them in mem_init() using boot_map_region().*/
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	if((uint32_t)round_up_end>UTOP)
	  panic("region_alloc: attempting to allocate physical memory above UTOP");
	struct PageInfo *pp;
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	if(elf_header->e_magic!=ELF_MAGIC)
	  panic("load_icode: trying to load badbinary image, magic elf not found :(");
...
env_run(struct Env *e)
{
	...
	unlock_kernel();                   //ANSWER LAB 4 Exercise 5  4/4
